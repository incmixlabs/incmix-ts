...
export default function DefaultTheme() {
  const previousLink: ContextualNavLink = {
    href: "/docs/features/incmix-factory",
    label: "Hope factory",
  };
  ...
  return (
    ...
    >
      ...
      <Text mb="$8">
        ...
        type scale, font stacks, border radius values, and more.
      </Text>
      <SectionTitle id="usage">Usage</SectionTitle>
      <Text mb="$5">
        Many CSS properties used in style props or stitches's <Code>css</Code>{" "}
        function are mapped to a scale from the Hope UI theme. The list of CSS
        property / theme token mapping is available on the{" "}
        <Anchor
      ...
      <Text mb="$5">
        The <Code>colors</Code> key allows you to customize the color palettes
        for your project.
      </Text>
      <Text mb="$5">
        Hope UI uses{" "}
        ...
        in its default theme. Radix Colors are designed to be accessible,
        well-balanced, harmonious and provide light and dark palette for each of
        their colors.
      </Text>
      <Alert status="warning" mb="$5">
        <AlertDescription>
          Since Hope UI components are built with Radix Colors, unless you have
          any design skills, the recommended way of customizing colors is to
          pick a light and dark scale from Radix Colors in order to keep all
          components accessible and dark mode friendly.
        </AlertDescription>
      </Alert>
      <CodeSnippet snippet={snippets.colors} mb="$8" />
      <Text mb="$8">
        Below are the list of all colors available in the default Hope UI theme.
        For semantic colors like <Code>primary</Code>, switching to dark mode
        will show the proper hex values from the dark palette.
      </Text>
      ...
      <Text mb="$2">
        To manage typography options, the theme object supports the following
        keys:
      </Text>
      <incmix.ul ps="$5" mb="$5">
        <incmix.li mb="$2">
      ...
      <Text mb="$5">
        The <Code>space</Code> key allows you to customize the global spacing
        scale for your project.
      </Text>
      <CodeSnippet snippet={snippets.space} mb="$5" />
      <Alert status="info" mb="$12">
        <AlertDescription>
          <Code>0_5</Code> actually means <Code>0.5</Code>, unfortunately
          stitches doesn't allow dots in theme token names.
        </AlertDescription>
      </Alert>
      ...
      <Text mb="$5">
        ...
        components you build for your project.
      </Text>
      <CodeSnippet snippet={snippets.sizes} mb="$12" />

      <SectionTitle id="radii">Radii (Border radius)</SectionTitle>
      <Text mb="$5">
        Hope UI provides a set of smooth corner radius values. The{" "}
        <Code>radii</Code> key allows you to customize the global radii scale
        for your project.
      </Text>
      ...
      <Text mb="$5">
        Hope UI provides a default set of shadows. The <Code>shadows</Code> key
        allows you to customize the global shadow scale for your project.
      </Text>
      <CodeSnippet snippet={snippets.shadows} mb="$12" />

      <SectionTitle id="z-indices">z-index values</SectionTitle>
      <Text mb="$5">
        incmix UI provides a set of z-indices out of the box to help control the
