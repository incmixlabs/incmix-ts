import { Badge, Box, incmix, Text } from "@incmix/ui";
...
export default function BoxDoc() {
  ...
  return (
    ...
    >
      ...
      <Text mb="$5">
        Box is the most abstract component on top of which all other Hope UI
        components are built. By default, it renders a <Code>div</Code> element.
      </Text>
      <Text mb="$2">
        The Box component is useful because it helps with 3 common use cases:
      </Text>
      <incmix.ul ps="$6" mb="$8">
        <incmix.li mb="$2">Create responsive layouts with ease.</incmix.li>
        <incmix.li mb="$2">
          Provide a shorthand way to pass styles via props.
        </incmix.li>
        <incmix.li>
          Compose new component and allow for override using the <Code>as</Code>{" "}
          prop.
        </incmix.li>
      </incmix.ul>
          ...
          <Box p="$6">
            ...
            <Box display="flex" mt="$2" alignItems="center">
              <For each={Array(5).fill("")}>
                {(_, i) => (
                  <IconStar
                    color={i() < property.rating ? "$warning9" : "$neutral3"}
                  />
                )}
              </For>
      ...
      <Text mb="$5">
        You can use the <Code>as</Code> prop to change the element render, just
