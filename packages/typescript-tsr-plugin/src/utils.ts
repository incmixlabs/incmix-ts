...
export function isTsrFilePath(filePath: string) {
  return filePath.endsWith(".tsr");
}

export function isVirtualTsrFilePath(filePath: string) {
  return filePath.endsWith(".tsr.ts");
}

export function toRealTsrFilePath(filePath: string) {
  return filePath.slice(0, -".ts".length);
}

export function ensureRealTsrFilePath(filePath: string) {
  return isVirtualTsrFilePath(filePath)
    ? toRealTsrFilePath(filePath)
    : filePath;
}
...
export function isInGeneratedCode(text: string, start: number, end: number) {
  ...
  const lastStart = text
    ...
    .lastIndexOf("/*Ωignore_startΩ*/");
  const lastEnd = text
    .substring(lineStart, start)
    .lastIndexOf("/*Ωignore_endΩ*/");
  return (
    lastStart > lastEnd &&
