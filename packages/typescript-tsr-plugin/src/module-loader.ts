...
import { createTsrSys } from "./svelte-sys";
import { ensureRealTsrFilePath, isVirtualTsrFilePath } from "./utils";
...
class ModuleResolutionCache {
  ...
  private getKey(moduleName: string, containingFile: string) {
    return containingFile + ":::" + ensureRealTsrFilePath(moduleName);
  }
}
...
export function patchModuleLoader(
  ...
  const svelteSys = createTsrSys(logger);
  ...
  function resolveModuleNames(
    ...
    // Tsr file and if so, are resolved, too. This way we can defer
    // all module resolving logic except for Tsr files to TypeScript.
    const resolved =
    ...
    return resolved.map((moduleName, idx) => {
      ...
      if (moduleName || !ensureRealTsrFilePath(fileName).endsWith(".svelte")) {
        return moduleName;
      }
  ...
  function resolveModuleName(
    ...
    if (
      ...
      !isVirtualTsrFilePath(svelteResolvedModule.resolvedFileName)
    ) {
      return svelteResolvedModule;
    }

    const resolvedFileName = ensureRealTsrFilePath(
      svelteResolvedModule.resolvedFileName
    );
    logger.log("Resolved", name, "to Tsr file", resolvedFileName);

    const resolvedTsrModule: ts.ResolvedModuleFull = {
      extension:
        /*snapshot.isTsFile
