...
export const resolveUnionOrIntersection: Resolver<ts.TypeReferenceNode> = (
  node
) =>
  encapsulateAccordingly(
    node,
    (checker.getTypeAtLocation(node) as UnionOrIntersectionType).types.map(
      (child) => resolve(child)
    ) as ts.TypeNode[]
  );
