...
export const visitTypeOperator: Visiter<ts.TypeOperatorNode> = ({
  node,
  metadata,
  deps,
}) => {
    ...
    case ReadonlyKeyword:
      ...
      return ts.factory.createObjectLiteralExpression(
        [
           ...
           * */
          ...(
            visit({ node: type, deps }) as ts.ObjectLiteralExpression
          ).properties.filter(
            (properties) =>
              ((properties as ts.PropertyAssignment).name as ts.StringLiteral)
                .text !== "itemsAreReadOnly"
          ),
          ts.factory.createPropertyAssignment(
            "itemsAreReadOnly",
