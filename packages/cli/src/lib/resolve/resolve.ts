...
export const resolve: Resolver = (node) => {
  ...
  console.log(
    "resolve",
    `ts.TypeFlags[${nodeType}]=${ts.TypeFlags[nodeType]}`,
    ts.isTypeReferenceNode(node as ts.Node) ? "Type Ref Node" : "Type"
  );

  if (resolutionMap[nodeType]) return resolutionMap[nodeType]!(node);
  else
    throw new Error(
      `Identifiers which reference the type: ${ts.TypeFlags[nodeType]} are not supported yet!`
    );
};
