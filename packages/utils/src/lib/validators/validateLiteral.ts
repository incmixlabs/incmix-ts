...
export const validateLiteral: TSRObjValidator<LiteralTSRObj> = (tsRuntimeObject, data) => {
    const primitiveLiterals: readonly string[] = [
        "number", "string", "boolean", "bigint"
    ];

    if (primitiveLiterals.includes(tsRuntimeObject.literalType) && tsRuntimeObject.value === data) {
        return Valid;
    } else {
