...
import {ConcreteTsRuntimeObject, UnionTsRuntimeObject, validateTsRuntimeObject} from "../../index";
import invalidWithChildren = Stack.invalidWithChildren;
import InvalidType = Stack.InvalidType;

export const validateUnion: TSRObjValidator<UnionTsRuntimeObject & ConcreteTsRuntimeObject> = (tsRuntimeObject, data) => {
    ...
    if (stackTraces.includes(Valid))
        return Valid;
    else
        return invalidWithChildren(tsRuntimeObject.type, stackTraces as InvalidType[]);
}