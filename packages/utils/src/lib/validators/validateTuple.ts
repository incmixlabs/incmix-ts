...
import {
    ConcreteTsRuntimeObject, TupleTsRuntimeObject, validateTsRuntimeObject,
} from "../../index";
...
export const validateTuple: TSRObjValidator<TupleTsRuntimeObject & ConcreteTsRuntimeObject> = (tsRuntimeObject, data) => {
    if (Array.isArray(data)) {
        const dataAsArray = data as any[];

        const stackTrace = Array(Math.max(tsRuntimeObject.items.length, dataAsArray.length))
            .fill(undefined)
            /*
            ...
            .map((_, i) =>
                validateTsRuntimeObject(
                    (i < tsRuntimeObject.items.length ?
                            ...
                            undefined
                    ) as ConcreteTsRuntimeObject, dataAsArray[i])
            )
            /*
